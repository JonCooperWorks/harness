...
- `go run ./cmd/harness -file my-plugin.encrypted -key harness_private.pem -president-key president_public.pem -args '{"message":"hi"}'` – quick manual smoke test that exercises the decrypt/execute flow.
- `go run ./cmd/sign -plugin examples/get-ip/target/wasm32-wasip1/release/get_ip_plugin.wasm -type wasm ...` – re-sign fixtures when plugin code changes.
...
## Testing Guidelines
Unit tests should live next to the code they cover (`foo_test.go`) and follow the `TestXxx` naming convention. Include table-driven tests for crypto edge cases and keystore adapters. For integration coverage, add new scenarios that sign, verify, and execute a WASM. Ensure `go test ./...` passes before opening a PR and include encrypted companions for any new `.wasm` fixtures.
