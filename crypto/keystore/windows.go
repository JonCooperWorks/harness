...
func init() {
	RegisterKeystore("windows", NewWindowsKeyManager)
}

// WindowsKeyManager implements KeyManager for Windows using Credential Manager.
...
// NewWindowsKeyManager creates a new Windows Credential Store key manager.
func NewWindowsKeyManager() (KeyManager, error) {
	...
	return &WindowsKeyManager{ring: ring}, nil
}

// SetPrivateKey stores an Ed25519 private key in Windows Credential Store.
func (w *WindowsKeyManager) SetPrivateKey(id KeyID, privateKey ed25519.PrivateKey) error {
...
func (w *WindowsKeyManager) ListKeys() ([]KeyID, error) {
...
func (w *WindowsKeyManager) ForKey(id KeyID) (Keystore, error) {
	// Verify the key exists by trying to retrieve it
	_, err := w.getPrivateKey(id)
	if err != nil {
		return nil, fmt.Errorf("key %s not found in credential store: %w", id, err)
	}
...
// getPrivateKey retrieves an Ed25519 private key from Windows Credential Store (internal helper).
func (w *WindowsKeyManager) getPrivateKey(keyID KeyID) (ed25519.PrivateKey, error) {
	item, err := w.ring.Get(string(keyID))
	if err != nil {
...
// windowsKeystore implements Keystore for Windows, bound to a specific key.
...
func (w *windowsKeystore) KeyID() KeyID {
	return w.keyID
}

// PublicKey returns the Ed25519 public key for this keystore's key.
func (w *windowsKeystore) PublicKey() (ed25519.PublicKey, error) {
...
func (w *windowsKeystore) PublicKeyX25519() ([32]byte, error) {
	privateKey, err := w.getPrivateKey()
	if err != nil {
		return [32]byte{}, err
	}
...
func (w *windowsKeystore) EncryptFor(recipientPub [32]byte, plaintext []byte, context Context) ([]byte, KeyID, error) {
	// Generate ephemeral Ed25519 key pair
	ephemeralPublic, ephemeralPrivate, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		return nil, w.keyID, fmt.Errorf("failed to generate ephemeral key: %w", err)
	}

	// Convert Ed25519 keys to X25519 for key exchange
	ephemeralX25519Private, err := Ed25519ToX25519PrivateKey(ephemeralPrivate)
	if err != nil {
		return nil, w.keyID, fmt.Errorf("failed to convert ephemeral private key to X25519: %w", err)
	}
	...
	if _, err := rand.Read(nonce); err != nil {
		return nil, w.keyID, fmt.Errorf("failed to generate nonce: %w", err)
	}

	block, err := aes.NewCipher(aesKey[:])
	if err != nil {
		return nil, w.keyID, fmt.Errorf("failed to create cipher: %w", err)
	}

	gcm, err := cipher.NewGCM(block)
	if err != nil {
		return nil, w.keyID, fmt.Errorf("failed to create GCM: %w", err)
	}
	...
	if err != nil {
		return nil, w.keyID, fmt.Errorf("failed to convert ephemeral public key to X25519: %w", err)
	}
	...
	return result, w.keyID, nil
}

// Decrypt decrypts ciphertext that was encrypted to this keystore's key.
func (w *windowsKeystore) Decrypt(ciphertext []byte, context Context) ([]byte, KeyID, error) {
	if len(ciphertext) < 32+12+16 { // Need at least ephemeral key (32) + nonce (12) + tag (16)
		return nil, w.keyID, errors.New("encrypted blob too short")
	}
	...
	privateKey, err := w.getPrivateKey()
	if err != nil {
		return nil, w.keyID, err
	}

	x25519PrivateKey, err := Ed25519ToX25519PrivateKey(privateKey)
	if err != nil {
		return nil, w.keyID, fmt.Errorf("failed to convert private key to X25519: %w", err)
	}
	...
	if len(encryptedData) < 12+16 { // Need at least nonce (12) + tag (16)
		return nil, w.keyID, errors.New("encrypted data too short")
	}
	...
	if err != nil {
		return nil, w.keyID, fmt.Errorf("failed to decrypt: %w", err)
	}

	return plaintext, w.keyID, nil
}

// getPrivateKey retrieves the Ed25519 private key for this keystore's key.
func (w *windowsKeystore) getPrivateKey() (ed25519.PrivateKey, error) {
	item, err := w.ring.Get(string(w.keyID))
	if err != nil {
		return nil, fmt.Errorf("failed to get key from credential store: %w", err)
	}
