...
func (k *keyringKeystore) Sign(msg, context Context) ([]byte, error) {
	privateKey, err := k.getPrivateKey()
	if err != nil {
		return nil, err
	}
...
func (k *keyringKeystore) Verify(pubKey ed25519.PublicKey, msg, sig, context Context) error {
	// Compute digest with domain separation: SHA-256(context || msg)
	h := sha256.New()
	h.Write(context)
	h.Write(msg)
...
func (k *keyringKeystore) EncryptFor(recipientPub [32]byte, plaintext []byte, context Context) ([]byte, KeyID, error) {
	...
	aesKey, err := deriveKeyFromSecretLinux(sharedSecret[:], context)
	if err != nil {
...
// deriveKeyFromSecretLinux derives a 32-byte AES key using HKDF-SHA256 with context.
func deriveKeyFromSecretLinux(sharedSecret []byte, context Context) ([32]byte, error) {
	paddedSecret := padSharedSecretLinux(sharedSecret)
...
// padSharedSecretLinux pads a shared secret to exactly 32 bytes for consistent key derivation.
func padSharedSecretLinux(secret []byte) []byte {
	const keySize = 32
